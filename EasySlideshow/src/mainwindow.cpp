    ...
    ui(new Ui::MainWindow)
{
    ...
    // set style
    ui->statusLabel->setMinimumWidth(64);
...
eventFilter(QObject *obj, QEvent *event)
{
    if (event->type() == QMouseEvent::MouseMove){
        ...
        int iMouseY = mouseEvent->pos().y();
        int iBottom = this->height() - (ui->statusLabel->height()*2);
        if ( iMouseY > iBottom ){ //mouse at bottom
            //qDebug() << iMouseY << iBottom;
