 ...
 int SlideshowImage::orientation() const
{
 ...
 QDateTime SlideshowImage::creationTime() const {
     ...
     return QFileInfo(_path).birthTime();
 }
...
int SlideshowImage::exif_orientation(QString fileName) {
        QFile fileRead(fileName);
        unsigned int fsize = (unsigned int)fileRead.size();
        char *buf = new char[fsize];

        QFile myFile(fileName);
        if (myFile.open(QIODevice::ReadOnly)){
            //fsize = myFile.size();  //when file does open.
            myFile.read(buf, fsize);
            myFile.close();

        // Parse exif
        EXIFInfo result;
        int err =ParseEXIF((unsigned char*)buf, fsize, result);
        if (err==0) {
            qDebug() << "Exif Orientation:" << result.orientation;
            ...
            qDebug() << "Exif Err:" << err << sErr;
            return err;
            }
        }
}

void SlideshowImage::exif_transform(const QString path, int angle)  {
    if (angle != 90 && angle != 180 && angle != 270)
        return;

    //int current_orientation = 0;
    int current_orientation = exif_orientation(path);
/*
